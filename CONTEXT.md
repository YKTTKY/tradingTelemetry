...
- **History depth for pan (polish ship = A2):** on chart interest, load a **deeper** initial series for short timeframes (especially **1m**) so panning can reach **prior session/day(s)** without edge-fetch — target on the order of **multiple RTH days** of 1m (exact bar cap TBD at implement, vs today’s ~500). Longer timeframes can keep smaller caps. **Edge fetch of older bars (B)** is **near-future**, async, not blocking pan keys.
